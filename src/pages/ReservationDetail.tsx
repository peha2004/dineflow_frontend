...
const { id, code } = useParams<{id?: string; code?: string;}>();
 ...
 useEffect(() => {
  ...
  if (!reservation) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-white via-purple-50 to-pink-50 flex flex-col items-center justify-center gap-4">
        <p className="text-gray-500 font-bold">Reservation not found.</p>
        <Link
          to="/reservations/my"
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-purple-50 to-pink-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-10 space-y-6">
          ...
          <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-green-600 mt-0.5">
                Your reservation has been created. Show the QR code at the
                restaurant.
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <span
            className={`text-sm px-3 py-1.5 rounded-full font-bold border ${STATUS_STYLES[r.status]}`}
          >
            {r.status}
          </span>
        </div>
        ...
        {qrCode && (
          <div className="bg-white border border-purple-100 rounded-3xl p-6 text-center shadow-md shadow-purple-100">
            ...
            <p className="text-xs text-gray-400 max-w-xs mx-auto">
              Show this QR code at the restaurant reception for quick check-in.
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {[
              ...
              [
                "🍽️ Table",
                table
                  ? `T${String(table.tableNumber).padStart(2, "0")} (${table.capacity} seats)`
                  : "—",
              ],
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between text-gray-600">
              ...
              <span className="font-semibold">
                Rs. {r.tableCharge?.toLocaleString()}
              </span>
            </div>
            {r.itemsTotal > 0 && (
              <div className="flex justify-between text-gray-600">
                ...
                <span className="font-semibold">
                  Rs. {r.itemsTotal?.toLocaleString()}
                </span>
              </div>
            )}
        ...
        <div className="flex gap-3">
          <Link
            to="/reservations/my"
            className="flex-1 text-center py-3 rounded-2xl border border-purple-200 text-purple-600 font-bold text-sm hover:bg-purple-50 transition"
          >
            ← All Reservations
          </Link>
          <Link
            to="/reservations/create"
            className="flex-1 text-center py-3 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-500 text-white font-bold text-sm hover:opacity-90 transition shadow-md shadow-purple-200"
          >
            + New Booking
          </Link>
        </div>
